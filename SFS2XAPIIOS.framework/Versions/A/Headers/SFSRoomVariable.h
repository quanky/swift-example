 ...
 */
@interface SFSRoomVariable : BaseVariable <RoomVariable>
{
	BOOL _isPersistent;
	BOOL _isPrivate;
}
...
/**
 @param name the name of the variable
 @param value the variable value ( can be BOOL, Integer, Double, String, SFSObject, SFSArray )
 @param isPrivate indicates whether the variable can be modified by other users or not
 @param isPersistent indicates whether the variable is removed from the Room when the user leaves (YES = variable is not removed)
 */
+(id)variableWithName:(NSString *)name value:(id)value isPrivate:(BOOL)priv isPersistent:(BOOL) pers;
